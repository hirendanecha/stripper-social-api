...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in Stripper Social channel by the Stripper Social Admin.
  To access your channel, log into your Stripper Social account,click on the
  Stripper Social icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `Stripper Social has approved your Connection application.`;
    } else {
      msg = `Stripper Social has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
